...
from .assignment import WorkAssignment
...
class BaseKFACPreconditioner:
    ...
    def step(self) -> None:
        ...
        if self.steps % self.inv_update_steps == 0:
            self.timer('reduce_invert_and_broadcast_factors', self.reduce_invert_and_broadcast_factors)

        # Compute Preconditioned Gradients
        self.timer('precondition', self.precondition_grads)

        self.timer('update_weights', self.kl_ckip_and_update_grad)
    ...
    def reduce_invert_and_broadcast_factors(self) -> None:
        if (
    ...
    def precondition_grads(self) -> None:
        for name, layer in reversed(list(self._layers.values())):
    ...
    def kl_ckip_and_update_grad(self):
