...
class MKOROptimizer(optim.Optimizer):
    def __init__(self,
        ...
        self.input_errors = np.zeros(20)
        self.grad_errors = np.zeros(20)
        self.input_mean_errors = np.zeros(20)
        self.grad_mean_errors = np.zeros(20)

        self.input_cov, self.grad_cov = [0] * len(self.modules), [0] * len(self.modules)
        with open("results/ranks.csv", "w") as f:
            string = make_csv_string(["Input Rank Average",
                                      "Input Rank Max",
                                      "Input Rank Min",
                                      "Input Rank Median"
                                      "Input Rank Ratio Average",
                                      "Grad Rank Average",
                                      "Grad Rank Max",
                                      "Grad Rank Min",
                                      "Grad Rank Median"
                                      "Grad Rank Ratio Average",
                                      "SVD Input Errors",
                                      "SVD Input Errors Min",
                                      "SVD Input Errors Max",
                                      "SVD Input Error Median",
                                      "SVD Input Exact Error"
                                      "SVD Grad Errors",
                                      "SVD Grad Errors Min",
                                      "SVD Grad Errors Max",
                                      "SVD Grad Error Median",
                                      "SVD Grad Exact Error",
                                      "Mean Input Error",
                                      "Mean Input Error Min",
                                      "Mean Input Error Max",
                                      "Mean Input Error Median",
                                      "Mean Grad Error",
                                      "Mean Grad Error Max"
                                      "Mean Grad Error Min",
                                      "Mean Grad Error Median"])
            f.write(string)
    ...
    def _save_input(self, module, input):
        if (torch.is_grad_enabled() and (self.steps % self.inv_freq == 0 or self.steps < self.warmup_steps)) and self.kfac:
            ...
            if self.accumulated_steps[self.index(module)] % self.grad_accum_steps == 0:
                ...
                self.input_cov[self.index(module)] = a.t() @ a
            else:
                try:
                    self.inputs[self.index(module)] += a.to(self.data_type)
                    self.input_cov[self.index(module)] += a.t() @ a
    ...
    def _save_grad_output(self, module, grad_input, grad_output):
        ...
        if (self.steps % self.inv_freq == 0 or self.steps < self.warmup_steps) and self.kfac:
            ...
            if self.accumulated_steps[self.index(module)] % self.grad_accum_steps == 0:
                ...
                self.grad_cov[self.index(module)] = g.t() @ g
            else:
                try:
                    self.grads[self.index(module)] += g.to(self.data_type)
                    self.grad_cov[self.index(module)] += g.t() @ g
    ...
    def compute_error(self, target, approx, p):
        return torch.norm(target - approx, p=p) / torch.norm(target, p=p)

    def compute_rank(self, mat, rank_threshold=0.8):
        U, S, V = torch.linalg.svd(mat, full_matrices=False)
        energy_distribution = S.clone().detach().cpu().numpy() / torch.sum(S).item()
        a = (U[:, 0].reshape(-1, 1) * S[0]) @ V[0, :].reshape(1, -1)
        error = torch.norm(mat - a, p=2)
        normalized_error = self.compute_error(mat, a, 2)
        S = torch.cumsum(S, dim=0) / torch.sum(S)
        rank = torch.argmax((S > rank_threshold).to(torch.float32)) + 1
        return rank.item(), mat.shape[0], error.item(), normalized_error.item(), energy_distribution

    def compute_mean_error(self, module):
        input_approx = self.inputs[self.index(module)].t() @ self.inputs[self.index(module)]
        grad_approx = self.grads[self.index(module)].t() @ self.grads[self.index(module)]
        input = self.input_cov[self.index(module)]
        grad = self.grad_cov[self.index(module)]
        input_error = self.compute_error(input, input_approx, p=2)
        grad_error = self.compute_error(grad, grad_approx, p=2)
        return input_error.item(), grad_error.item()

    def compute_errors(self):
        input_exact_errors , grad_exact_errors = [], []
        input_ranks, grad_ranks = [], []
        input_rank_ratios, grad_rank_ratios = [], []
        normalized_errors_2 = []
        normalized_errors_inf = []
        input_errors, grad_errors = [], []
        mean_input_errors, mean_grad_errors = [], []
        if not hasattr(self, "energy_distributions"):
            max_dim = 0
            for m in self.modules:
                max_dim = max(max_dim, self.input_cov[self.index(m)].shape[0], self.grad_cov[self.index(m)].shape[0])
            import numpy as np
            self.input_energy_distributions = np.zeros(max_dim)
            self.grad_energy_distributions = np.zeros(max_dim)
            self.num_samples = 0

        for m in self.modules:
            input_rank, input_dim, input_error, input_normalized_error, S = self.compute_rank(
                self.input_cov[self.index(m)])
            self.input_energy_distributions[0:len(S)] += S
            grad_rank, grad_dim, grad_error, grad_normalized_error, S = self.compute_rank(
                self.grad_cov[self.index(m)])
            self.grad_energy_distributions[0:len(S)] += S
            self.num_samples += 1

            input_ranks += [input_rank]
            grad_ranks += [grad_rank]
            input_rank_ratios += [input_rank / input_dim]
            grad_rank_ratios += [grad_rank / grad_dim]

            input_errors += [input_normalized_error]
            grad_errors += [grad_normalized_error]
            input_exact_errors += [input_error]
            grad_exact_errors += [grad_error]

            input_error, grad_error = self.compute_mean_error(m)
            mean_input_errors += [input_error]
            mean_grad_errors += [grad_error]

        def extract_stats(lst):
            return [average(lst), max(lst), min(lst), median(lst)]



        with open("./results/ranks.csv", "a") as f:
            string = make_csv_string(extract_stats(input_ranks) +
                                     [average(input_rank_ratios)] +
                                     extract_stats(grad_ranks) +
                                     [average(grad_rank_ratios)] +
                                     extract_stats(input_errors) +
                                     [average(input_exact_errors)] +
                                     extract_stats(grad_errors) +
                                     [average(grad_exact_errors)] +
                                     extract_stats(mean_input_errors) +
                                     extract_stats(mean_grad_errors))
            f.write(string)
        self.input_errors += np.histogram(input_errors, bins=20, range=(0, 1))[0]
        self.grad_errors += np.histogram(grad_errors, bins=20, range=(0, 1))[0]
        self.input_mean_errors += np.histogram(mean_input_errors, bins=20, range=(0, 1))[0]
        self.grad_mean_errors += np.histogram(mean_grad_errors, bins=20, range=(0, 1))[0]


        def plot_stairs(x, y, filename):
            plt.figure()
            plt.stairs(y, x)
        ...
        torch.save([input_ranks, grad_ranks, input_errors, grad_errors, mean_input_errors, mean_grad_errors, self.input_energy_distributions, self.grad_energy_distributions,  self.num_samples], f"./results/stats_{self.num_samples}.pt")
        plot_stairs(range(1, 32), self.input_energy_distributions[0:30] / self.num_samples, "./results/input_energy_distirution.pdf")
        plot_stairs(range(1, 32), self.grad_energy_distributions[0:30] / self.num_samples, "./results/grad_energy_distirution.pdf")
        plot_stairs(x, self.input_errors, "./results/input_errors.pdf")
        plot_stairs(x, self.grad_errors, "./results/grad_errors.pdf")
        plot_stairs(x, self.input_mean_errors, "./results/input_mean_errors.pdf")
        plot_stairs(x, self.grad_mean_errors, "./results/grad_mean_errors.pdf")
